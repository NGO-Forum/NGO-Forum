// src/admin/components/documents/DocumentForm.jsx
import React, { useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import { api } from "../../../API/api";
import StatusModal from "../StatusModal"; // <-- import your modal

export default function DocumentForm({ editDoc, setEditDoc, onSuccess }) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editDoc) {
        ...
        await api.post(`/documents/${editDoc.id}`, form, {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {isEditing && editDoc.file_kh && (
            <a
              href={`${import.meta.env.VITE_APP_URL}/storage/${editDoc.file_kh}`}
              target="_blank"
        ...
        <div className="space-y-2">
          ...
          {isEditing && editDoc.file_en && (
            <a
              href={`${import.meta.env.VITE_APP_URL}/storage/${editDoc.file_en}`}
              target="_blank"
              className="text-xs text-blue-600 underline"
            >
